...
@else
    <form action="{{ url('/peran/' . $peran->peran_id . '/delete_ajax') }}" method="POST" id="form-delete">
        @csrf
        @method('DELETE')
        <div id="modal-master" class="modal-dialog modal-lg" role="document">
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header bg-danger text-white">
                    <h5 class="modal-title" id="exampleModalLabel">
                        <i class="fas fa-trash-alt mr-2"></i>Hapus Data Peran
                    </h5>
                    <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                ...
                <div class="modal-body p-4">
                    ...
                    <table class="table table-hover">
                        <tr>
                            <th class="text-right" style="width: 30%;">Kode Peran:</th>
                            <td style="width: 70%;">
                                <span class="badge badge-primary">{{ $peran->peran_kode }}</span>
                            </td>
                        </tr>
                        <tr>
                            <th class="text-right">Nama Peran:</th>
                            <td>
                                <strong>{{ $peran->peran_nama }}</strong>
                            </td>
                        </tr>
                    </table>
                </div>
